$(document).ready(function() {
    ...
    if ($('#phoneNumber').length && typeof Cleave !== 'undefined') {
    ...
    // Modal triggers
    $('#addStudentBtn').on('click', () => openModal(modals.addStudent));
    $('#closeAddModal, #cancelAddStudent').on('click', () => closeModal(modals.addStudent));
    ...
    $('#profile-photo').on('change', function() {
        if (this.files.length > 0) {
            ...
            reader.onload = (e) => $('#profile-photo-preview').attr('src', e.target.result);
            reader.readAsDataURL(this.files[0]);
        }
    });

    // Form submission
    const $registerForm = $('#save-student-form');
