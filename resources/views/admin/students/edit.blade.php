            ...
            <div class="text-center -mt-12 sm:-mt-16">
                <div class="relative inline-block">
                    ...
                    <div class="absolute bottom-0 right-0">
                        ...
                        @else
                            <form id="update-profile-picture-form" action="{{ route('admin.students.picture.update', $student->id) }}" method="POST" enctype="multipart/form-data" class="inline">
                                @csrf
                ...
                <h4 class="text-lg sm:text-xl font-medium text-[#1B1B1B] mt-3 sm:mt-4">{{ $student->name }}</h4>
                <p class="text-xs sm:text-sm text-[#6B7280]">{{ $student->email }}</p>
            ...
            <div class="col-span-1 md:col-span-4 lg:col-span-4">
                <div class="sticky top-20">
                    <div class="bg-white rounded-[20px] md:rounded-[30px] px-4 sm:px-6 md:px-8 py-4 sm:py-6 md:py-8 shadow-sm overflow-hidden">
                        <ul class="space-y-4">
                            <li>
                                <a class="flex items-center space-x-3 p-2 rounded-lg {{ request()->query('tab', 'personal') == 'personal' ? 'bg-[#F5CE9F] text-[#1B1B1B]' : 'text-[#1B1B1B] hover:bg-[#F5CE9F]' }} transition" href="{{ route('admin.students.edit', ['student' => $student->id, 'tab' => 'personal']) }}">
                            ...
                            <li>
                                <a class="flex items-center space-x-3 p-2 rounded-lg {{ request()->query('tab', 'personal') == 'security' ? 'bg-[#F5CE9F] text-[#1B1B1B]' : 'text-[#1B1B1B] hover:bg-[#F5CE9F]' }} transition" href="{{ route('admin.students.edit', ['student' => $student->id, 'tab' => 'security']) }}">
                                    <div class="w-12 h-12 rounded-full bg-[#E68815] flex items-center justify-center text-white">
                            ...
                            <li>
                                <a class="flex items-center space-x-3 p-2 rounded-lg {{ request()->query('tab') == 'social' ? 'bg-[#F5CE9F] text-[#1B1B1B]' : 'text-[#1B1B1B] hover:bg-[#F5CE9F]' }} transition" href="{{ route('admin.students.edit', ['student' => $student->id, 'tab' => 'social']) }}">
                                    <div class="w-12 h-12 rounded-full bg-[#E68815] flex items-center justify-center text-white">
            ...
            <div class="col-span-1 md:col-span-8 lg:col-span-8">
                <div class="bg-white rounded-[20px] md:rounded-[30px] px-4 sm:px-6 md:px-8 py-4 sm:py-6 md:py-8 shadow-sm overflow-hidden">
                    @if (request()->query('tab', 'personal') == 'personal')
                        <form id="profile-form" method="POST" action="{{ route('admin.students.profile.update', $student->id) }}" enctype="multipart/form-data" class="flex flex-col h-full">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="mb-5">
                                    ...
                                    <input id="name" type="text" class="w-full px-3 py-2 sm:px-4 sm:py-3 border border-[#E1E1E1] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#EB8C22] focus:border-0 text-[#1B1B1B] placeholder:text-[#6B7280]" name="name" value="{{ old('name', $student->name) }}" placeholder="Enter your full name" autocomplete="family-name">
                                    @error('name')
                                ...
                                <div class="mb-5">
                                    ...
                                    <input id="email" type="email" class="w-full px-3 py-2 sm:px-4 sm:py-3 border border-[#E1E1E1] rounded-lg text-[#1B1B1B] bg-gray-100 cursor-not-allowed" value="{{ $student->email }}" readonly placeholder="Enter your email address" autocomplete="email">
                                </div>
                    ...
                    @elseif (request()->query('tab', 'personal') == 'security')
                        <form id="security-form" method="POST" action="{{ route('admin.students.reset.password', $student->id) }}" class="flex flex-col h-full">
                            @csrf
                            ...
                            <div class="space-y-4">
                                ...
                                @if ($student->hasSocialAccount('google'))
                                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 sm:p-4 border border-[#E1E1E1] rounded-lg">
                                        ...
                                        <form action="{{ route('social.disconnect', ['provider' => 'google', 'student' => $student->id]) }}" method="POST" class="w-full sm:w-auto">
                                            @csrf
