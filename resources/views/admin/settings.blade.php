        ...
        <div class="w-auto bg-white rounded-[20px] md:rounded-[30px] px-6 md:px-8 py-6 md:py-8 shadow-sm overflow-hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-20">
                ...
                <div class="flex flex-col h-full">
                    ...
                    <form class="flex flex-col flex-1" id="profile-form" action="{{ route('admin.settings.update.profile') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-5">
                            ...
                            <input type="email" id="email" placeholder="Email" class="w-full px-4 py-3 border border-[#E1E1E1] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#EB8C22] focus:border-0 text-[#1B1B1B] placeholder:text-[#6B7280]" value="{{ old('email', auth()->user()->email) }}" readonly />
                            @error('email')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex flex-col h-full">
                    ...
                    <!-- Form Fields -->
                    <form class="flex flex-col flex-1 space-y-4" id="reset-password-form" action="{{ route('admin.settings.reset.password') }}" method="POST">
                        @csrf
                        <!-- Old Password -->
                        <div class="space-y-2">
