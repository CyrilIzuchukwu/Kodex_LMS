...
@section('content')
    ...
    <div class="max-w-7xl mx-auto ">
        ...
        <div class="w-auto bg-white rounded-[20px] px-4 py-4 md:px-6 md:py-6 shadow-sm overflow-hidden">
            ...
            <div>
                <form id="photos-videos-form" action="{{ route('admin.courses.edit.update.photos.videos', $course->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Load existing media data from session or database
            function loadExistingMedia() {
                ...
                if (existingMediaScript) {
                    try {
                        ...
                        if (sessionData) {
                            if (sessionData.course_photo && Array.isArray(sessionData.course_photo)) {
                                // Expected structure: { course_photo: [...], video_url: '' }
                                photos = sessionData.course_photo;
                            } else if (sessionData.image_url) {
                                // Current structure: { id: 1, course_id: 1, image_url: '...' }
                                photos = [sessionData.image_url];
                            }
            ...
            // Display an existing photo from session or database
