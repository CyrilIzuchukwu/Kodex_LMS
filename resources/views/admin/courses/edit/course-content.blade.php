...
@section('content')
    @php
        $modules = session('course.edit.content.modules', []);
    ...
    <div class="max-w-7xl mx-auto ">
        ...
        <div class="w-auto bg-white rounded-[20px] px-4 py-4 md:px-6 md:py-6 shadow-sm overflow-hidden">
            ...
            <div>
                <form id="course-content-form" action="{{ route('admin.courses.edit.update.content', $course->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div id="modules-container">
                        @foreach ($modules as $index => $mod)
                            @php
                                $key = $index + 1;
                            @endphp
                            <div class="border rounded-md overflow-hidden mb-6 module-item" data-module-id="{{ $key }}" data-db-id="{{ $mod['id'] ?? '' }}">
                                ...
                                <div class="moduleContent px-4 py-4 transition-all duration-500 ease-in-out max-h-[1000px] overflow-hidden">
                                    @if (isset($mod['id']))
                                        <input type="hidden" name="modules[{{ $key }}][id]" value="{{ $mod['id'] }}">
                                    @endif
                                    <!-- YouTube Video URL -->
                                    <div>
                                            ...
                                            <div class="preview-container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full md:w-2/3 module-preview-{{ $key }}">
                                                @if (!empty($mod['resources']))
                                                    @foreach ($mod['resources'] as $resource)
                                                        ...
                                                        @endphp
                                                        <div class="file-preview flex flex-col justify-between p-4 bg-white rounded-xl border border-gray-300 shadow-sm" data-path="{{ $resource }}">
                                                            <div class="w-full">
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-8 w-full">
                        <a href="{{ route('admin.courses.edit.photos.videos', $course) }}" id="prev-button" class="bg-[#EDEDED] block text-center w-full text-gray-800 text-sm font-medium px-6 py-3 rounded-full hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300">
                            Prev
                        </a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function addNewModule() {
                moduleCount++;
                const moduleHTML = `
                    <div class="border rounded-md overflow-hidden mb-6 module-item" data-module-id="${moduleCount}" data-db-id="">
                        <div class="flex items-center justify-between px-4 py-3 bg-gray-100 cursor-pointer toggleModule">
            ...
            function initializeModule(moduleId) {
                ...
                removeBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if (moduleCount > 1) {
                        const dbId = moduleElement.dataset.dbId;
                        if (dbId) {
                            const hiddenInput = document.createElement('input');
                            hiddenInput.type = 'hidden';
                            hiddenInput.name = 'delete_modules[]';
                ...
                function createPreviewElement(file) {
                    ...
                    return `
                        <div class="file-preview flex flex-col justify-between p-4 bg-white rounded-xl border border-gray-300 shadow-sm" data-filename="${file.name}" data-path="">
                            <div class="w-full">
                                <div class="flex justify-between items-center w-full space-x-1 mb-2">
                                    <div class="flex items-center space-x-1 overflow-hidden">
                ...
                existingRemoveButtons.forEach(btn => {
                    btn.addEventListener('click', function() {
                        ...
                        const resourcePath = previewElement.dataset.path;
                        previewElement.remove();
                        if (resourcePath) {
                            const hiddenInput = document.createElement('input');
                            hiddenInput.type = 'hidden';
                            hiddenInput.name = `modules[${moduleId}][remove_resources][]`;
                            hiddenInput.value = resourcePath;
                            form.appendChild(hiddenInput);
                        }
                    });
                });
            ...
            function renumberModules() {
                ...
                modules.forEach((module) => {
                    ...
                    const idInput = module.querySelector('input[name$="[id]"]');
                    if (idInput) {
                        idInput.name = `modules[${newCount}][id]`;
                    }
                    ...
                    removeInputs.forEach(input => {
                        const oldIdMatch = input.name.match(/modules\[(\d+)]/);
                        if (oldIdMatch) {
                            const oldId = parseInt(oldIdMatch[1]);
                            if (oldId > newCount) {
                                input.name = input.name.replace(`modules[${oldId}]`, `modules[${oldId - 1}]`);
                            }
                        }
                    });
                });
