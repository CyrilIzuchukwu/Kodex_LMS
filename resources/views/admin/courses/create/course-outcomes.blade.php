    ...
    <div class="mb-6 px-4 md:px-6">
        <nav class="bg-white rounded-[20px] md:rounded-[30px] shadow-sm px-4 md:px-6 py-3 flex items-center justify-start w-full">
            <ol class="flex items-center space-x-2 md:space-x-3 text-sm md:text-base font-medium text-[#141B34]">
                ...
                <li>
                    <span class="text-[#E68815] font-semibold">Course Outcomes</span>
                </li>
            </ol>
        </nav>
    </div>
    ...
    <div class="max-w-7xl mx-auto ">
        ...
        <div class="w-auto bg-white rounded-[20px] px-4 py-4 md:px-6 md:py-6 shadow-sm overflow-hidden ">
            ...
            <p class="text-gray-600 text-sm mb-6">
                Enter what students will learn in your course. Press comma or enter to add tags.
            </p>

            <form id="course-outcomes-form" action="{{ route('admin.courses.add.store.outcomes') }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-4 mt-8 w-full">
                    <a href="{{ route('admin.courses.add.details') }}" class="bg-[#EDEDED] w-full text-gray-800 text-sm font-medium px-6 py-3 rounded-full hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300">
                        Prev
                    </a>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Load existing objectives from session data
            function loadExistingObjectives() {
                ...
                if (existingObjectivesScript) {
                    try {
                        const sessionData = JSON.parse(existingObjectivesScript.textContent);
                        if (sessionData && sessionData.learning_objectives && Array.isArray(sessionData.learning_objectives)) {
                            tags = [...sessionData.learning_objectives]; // Create a copy of the array
                            updateTagDisplay();
