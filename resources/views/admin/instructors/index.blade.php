...
@section('content')
    <div class="mb-6">
        <nav class="bg-white rounded-[20px] md:rounded-[30px] shadow-sm px-4 md:px-6 py-3 flex items-center justify-start w-full">
            <ol class="flex items-center space-x-2 md:space-x-3 text-sm md:text-base font-medium text-[#141B34]">
                ...
                <li>
                    <span class="text-[#E68815] font-semibold">Instructor</span>
                </li>
            </ol>
        </nav>
    </div>
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4 mt-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full">
            ...
            <div class="flex-shrink-0 w-full sm:w-auto">
                <button id="addInstructorBtn" class="bg-[#E68815] px-4 md:px-6 py-3 rounded-full text-sm md:text-base font-medium text-white hover:bg-[#cc6f0f] transition-colors w-full sm:w-auto">
                    + Add Instructor
                </button>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 border-collapse">
                <thead class="bg-[#EDEDED]">
                    <tr>
                        ...
                        <th class="table-header px-2 md:px-6 py-3 text-xs md:text-sm font-medium text-gray-500 text-left">Instructor Name</th>
                ...
                <tbody class="bg-[#fcfafa] divide-y divide-gray-200" id="users-container">
                    @include('admin.instructors.instructor-items', ['users' => $users])
                </tbody>
            </table>
        </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-75 backdrop-blur-sm flex items-center justify-center z-[9999] hidden p-4">
        <div class="modal-content bg-white rounded-[20px] md:rounded-[30px] shadow-lg w-full max-w-sm md:max-w-md h-auto p-4 md:p-6 flex flex-col items-center justify-center z-[10000]">
            ...
            <h2 class="text-base md:text-lg font-semibold text-gray-800 mb-4 text-center">Delete Instructor?</h2>
            <p class="text-gray-600 mb-6 text-center text-xs md:text-sm">
                Are you sure you want to remove this instructor from the system? This action cannot be undone.
            </p>
            ...
            function setupDeleteModal() {
                ...
                document.querySelectorAll('.delete-user').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        const deleteUrl = `{{ route('admin.instructors.destroy', ':id') }}`.replace(':id', userId);
                ...
                deleteForm.addEventListener('submit', function(e) {
                    ...
                    fetch(this.action, {
                        ...
                        .catch(error => {
                            iziToast.error({
                                ...iziToastSettings,
                                message: error.message || 'An error occurred while deleting the instructor'
                            });
                        })
                        .finally(() => {
