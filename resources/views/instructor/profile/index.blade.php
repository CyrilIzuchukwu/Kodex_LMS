...
@section('content')
    <div class="mb-6 px-2 md:px-3">
        <nav class="bg-white rounded-[20px] md:rounded-[30px] shadow-sm px-4 md:px-6 py-3 flex items-center justify-start w-full">
            <ol class="flex items-center space-x-2 md:space-x-3 text-sm md:text-base font-medium text-[#141B34]">
                <li>
                    <a href="{{ route('instructor.dashboard') }}" class="hover:text-[#E68815] transition-colors duration-200 flex items-center">
    ...
    <div class="container mx-auto mt-4 mb-5" id="main-content">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="w-full bg-white rounded-[20px] md:rounded-[30px] px-6 md:px-8 py-6 md:py-8 shadow-sm overflow-hidden">
                <form class="flex flex-col h-full" id="profile-form" action="{{ route('instructor.profile.update.profile') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" id="phone_number" name="phone_number" placeholder="Phone Number" class="w-full px-4 py-3 border border-[#E1E1E1] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#EB8C22] focus:border-0 text-[#1B1B1B] placeholder:text-[#6B7280]" value="{{ old('phone_number', auth()->user()->profile?->phone_number) }}" />
                        @error('phone_number')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" id="address" name="address" placeholder="Address" class="w-full px-4 py-3 border border-[#E1E1E1] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#EB8C22] focus:border-0 text-[#1B1B1B] placeholder:text-[#6B7280]" value="{{ old('address', auth()->user()->profile?->address) }}" />
                        @error('address')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-5">
                        <label for="biography" class="block text-sm font-medium text-gray-600 mb-1">Biography</label>
                        <textarea id="biography" name="biography" placeholder="Tell us about yourself" class="w-full px-4 py-3 border border-[#E1E1E1] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#EB8C22] focus:border-0 text-[#1B1B1B] placeholder:text-[#6B7280] resize-y" rows="4">{{ old('biography', auth()->user()->profile?->biography) }}</textarea>
                        @error('biography')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="w-full bg-white rounded-[20px] md:rounded-[30px] px-6 md:px-8 py-6 md:py-8 shadow-sm overflow-hidden">
                <form class="flex flex-col h-full" id="reset-password-form" action="{{ route('instructor.profile.reset.password') }}" method="POST">
                    @csrf
                    <!-- Header -->
                    <div class="flex items-center space-x-3 mb-12">
                        <div class="w-12 h-12 rounded-full bg-[#E68815] flex items-center justify-center text-white">
    ...
    <div id="deleteModal" class="fixed hidden inset-0 bg-black bg-opacity-75 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 opacity-0 transition-all duration-300 ease-in-out">
        <div class="modal-content bg-white rounded-[20px] md:rounded-[30px] shadow-lg w-full max-w-sm md:max-w-md h-auto p-4 md:p-6 flex flex-col items-center justify-center z-[10000] transform scale-95 transition-transform duration-300 ease-in-out">
            ...
            <form id="delete-form" action="{{ route('instructor.profile.destroy') }}" method="POST">
                @csrf
                @method('DELETE')
