    ...
    <div class="container">
        ...
        <div class="glass-effect header">
            <div class="header-content">
                <a href="{{ route('instructor.courses.manage', $quiz->course->id) }}" class="back-btn">
                    <i class="mdi mdi-arrow-left"></i>
                </a>

                <div style="flex: 1;">
                    <h1>Edit Quiz</h1>
                    <p>Module: {{ $quiz->module->title }}</p>
                </div>
            </div>
        </div>
                ...
                <form id="quiz-form">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="quiz-title" name="title" class="form-input text-gray-900" value="{{ $quiz->title }}" placeholder="e.g., Advanced React Hooks Assessment" required>
                        </div>

                        <div class="form-group">
                            <label for="quiz-description" class="form-label">Description</label>
                            <textarea id="quiz-description" name="description" class="form-input text-gray-900 form-textarea" placeholder="Describe what this quiz covers and any important instructions for students...">{{ $quiz->description }}</textarea>
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                            <div class="form-group">
                                <label for="time-limit" class="form-label">Time Limit (minutes)</label>
                                <input type="number" id="time-limit" name="time_limit" class="text-gray-900 form-input" value="{{ $quiz->time_limit ?? 20 }}" min="1" max="180">
                            </div>

                            <div class="form-group">
                                <label for="pass-percentage" class="form-label">Passing Score (%)</label>
                                <input type="number" id="pass-percentage" name="pass_percentage" class="text-gray-900 form-input" value="{{ $quiz->pass_percentage }}" min="0" max="100" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        <h2 class="section-title">
                            ...
                            Quiz Questions
                            <span id="question-count" class="stat-value">({{ count($quiz->questions) }})</span>
                        </h2>
                        ...
                        <div id="questions-container">
                            ...
                            @else
                                <!-- Hide empty state when questions exist -->
                                <div id="empty-state" class="question-builder" style="display: none;">
                                    <div class="empty-state">
                                        <i class="mdi mdi-help-circle-outline"></i>
                                        <h3>No questions yet</h3>
                                        <p>Click "Add Question" to start building your quiz</p>
                                    ...
                                    <div class="question-card" id="question-{{ $index + 1 }}">
                                        <div class="question-header">
                                            ...
                                            <div class="question-actions">
                                                <button type="button" class="btn-icon danger" onclick="confirmDeleteQuestion('question-{{ $index + 1 }}')" title="Delete Question">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-input form-textarea text-gray-900" name="questions[{{ $index }}][text]" placeholder="Enter your question here..." required oninput="updateStats()">{{ $question->question_text }}</textarea>
                                        </div>

                                        <div class="form-group">
                                            <label class="form-label">Answer Options</label>
                                            <div class="options-container" id="options-question-{{ $index + 1 }}">
                                                @foreach($options as $optIndex => $option)
                                                    <div class="option-input">
                                                        <input type="radio" name="questions[{{ $index }}][correct]" value="{{ $optIndex }}" class="option-radio" {{ $option['correct'] ? 'checked' : '' }} required>
                                                        <input type="text" class="option-text text-gray-900" name="questions[{{ $index }}][options][{{ $optIndex }}]" value="{{ $option['text'] }}" placeholder="Option {{ chr(65 + $optIndex) }}" required>
                                            ...
                                            <button type="button" class="btn-add-option" onclick="addOption('options-question-{{ $index + 1 }}', {{ $index }})">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-input text-gray-900" name="questions[{{ $index }}][explanation]" placeholder="Provide an explanation for the correct answer..." rows="3">{{ $question->explanation }}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                            @endif
                ...
                <div class="glass-effect sidebar-card">
                    ...
                    <div class="quiz-stats">
                        <div class="stat-item">
                            ...
                            <span class="stat-value" id="total-questions">{{ count($quiz->questions) }}</span>
                        </div>

                        <div class="stat-item">
                            <span class="stat-label">Estimated Time</span>
                            <span class="stat-value" id="estimated-time">{{ max(count($quiz->questions) * 1.5, 5) }} min</span>
                        </div>

                        <div class="stat-item">
                            <span class="stat-label">Difficulty</span>
                            <span class="stat-value" id="difficulty-level">
                                @if(count($quiz->questions) >= 20)
                                    Hard
                                @elseif(count($quiz->questions) >= 10)
                                    Medium
                                @else
                                    Easy
                                @endif
...
@push('scripts')
    <script>
        ...
        quizForm.addEventListener('submit', function(e) {
            ...
            // Send AJAX request
            fetch('{{ route("instructor.courses.module.quiz.update", $quiz->id) }}', {
                method: 'POST',
                headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            window.location.href = '{{ route("instructor.courses.manage", $quiz->course->id) }}';
                        }, 2000);
                    } else {
                ...
                .catch(error => {
                    ...
                    showError(error.message || 'An error occurred while updating the quiz.');
                })
                .finally(() => {
        ...
        // Initialize stats on a page load
        updateStats();
    </script>
@endpush
