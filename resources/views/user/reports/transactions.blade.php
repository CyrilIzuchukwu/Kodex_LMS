                            ...
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-800 mt-1">
                                    {{ $payments->sum(function ($payment) {
                                        return count(json_decode($payment->cart_items, true) ?? []);
                                    }) }}
                                </p>
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-800 mt-1">
                                    ₦{{ number_format($payments->where('created_at', '>=', now()->startOfMonth())->sum('amount'), 2) }}
                                </p>
                            </div>
                        ...
                        <table class="w-full">
                            ...
                            @forelse ($payments as $payment)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-400 mt-1">TXN-{{ $payment->id }}-{{ date('Ymd', strtotime($payment->created_at)) }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $payment->status == 'completed' ? 'bg-green-100 text-green-800' : ($payment->status == 'processing' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                                            <span class="w-1.5 h-1.5 {{ $payment->status == 'completed' ? 'bg-green-500' : ($payment->status == 'processing' ? 'bg-yellow-500 animate-pulse' : 'bg-red-500') }} rounded-full mr-2"></span>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            @elseif ($payment->status == 'processing')
                                                <a href="{{ route('user.reports.transactions.show', $payment->id) }}" class="bg-gray-100 text-gray-400 px-3 py-2 rounded-lg text-xs font-medium cursor-not-allowed" disabled>
                                                    View Invoice
                                                </a>
                                            @else
                                                <a href="{{ route('user.cart')  }}" class="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200">
                                                    Retry Payment
                                                </a>
                                            @endif
                                        </div>
