<div class="glass-effect rounded-2xl p-4 sm:p-6 shadow-lg">
    ...
    <div class="flex flex-wrap gap-2 sm:gap-3 mb-6">
        <a href="{{ route('user.course.watch', ['course' => $course->id, 'module' => $current_module_id, 'tab' => 'qa']) }}"
           class="{{ request()->query('tab', 'qa') == 'qa' ? 'tab-active' : 'text-gray-600 bg-gray-100 hover:bg-gray-200' }} rounded-full px-4 sm:px-8 py-2 sm:py-3 font-medium text-xs sm:text-sm transition-all duration-300"
           data-tab="qa">
            Q&A
        </a>
        <a href="{{ route('user.course.watch', ['course' => $course->id, 'module' => $current_module_id, 'tab' => 'notes']) }}"
           class="{{ request()->query('tab', 'qa') == 'notes' ? 'tab-active' : 'text-gray-600 bg-gray-100 hover:bg-gray-200' }} rounded-full px-4 sm:px-8 py-2 sm:py-3 font-medium text-xs sm:text-sm transition-all duration-300"
           data-tab="notes">
            Notes
        </a>
    </div>
    ...
    <!-- Q&A Tab Content -->
    @if(request()->query('tab', 'qa') == 'qa')
        <div id="qa-tab" class="tab-content">
            <div id="questions-view" class="{{ request()->query('question_id') ? 'hidden' : '' }}">
                <!-- Search and Filters -->
                <form id="question-filter-form" method="POST" action="{{ route('user.course.questions.fetch', ['course' => $course->id]) }}">
                    @csrf
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
                    ...
                    <button onclick="toggleQuestionForm()" id="ask-question-btn" class="btn-primary text-white px-4 sm:px-6 py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300">
                        Ask Question
                    </button>
                </div>

                <!-- Ask Question Form -->
                <div id="ask-question-form" class="hidden bg-gray-50 rounded-xl p-4 sm:p-6 mb-6 sm:mb-8 relative">
                    <form method="POST" id="question-form" action="{{ route('user.course.questions.store', $course->id) }}">
                        @csrf
                        <input type="hidden" name="module_id" value="{{ $current_module_id }}">

                        <div class="mb-4">
                            <input type="text" name="title" id="question-title" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="What's your question about?" value="{{ old('title') }}">
                            <span class="text-red-500 text-xs hidden error-message" id="title-error">{{ $errors->first('title') }}</span>
                        </div>

                        <div class="mb-4">
                            <textarea name="question" id="question-content" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500" rows="4" placeholder="Describe your question in detail. Be specific about what you're trying to achieve or understand...">{{ old('content') }}</textarea>
                            <span class="text-red-500 text-xs hidden error-message" id="question-content-error">{{ $errors->first('content') }}</span>
                        </div>

                        <div class="flex justify-end gap-3">
                            <button type="button" onclick="toggleQuestionForm()" class="bg-gray-200 text-gray-700 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-sm sm:text-base">
                                Cancel
                            </button>

                            <button type="submit" class="btn-primary text-white px-6 sm:px-8 py-2 sm:py-3 rounded-full font-medium text-sm sm:text-base transition-all duration-300 flex items-center gap-2" id="question-submit-btn">
                                <span class="submit-text">Post Question</span>
                    ...
                    <div id="question-form-loading" class="hidden loading-overlay absolute inset-0 bg-gray-50 bg-opacity-75 flex items-center justify-center">
                        <div class="flex items-center gap-3 text-orange-600">
                            <svg class="animate-spin h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
                            </svg>
                            <span class="font-medium">Posting your question...</span>
                        </div>
                    </div>
                </div>
                ...
                <div id="questions-container" class="space-y-4 sm:space-y-6 overflow-y-auto smooth-scroll max-h-[60vh]">
                    @include('user.courses.watch.navigation.questions', ['questions' => $questions])
                </div>
            ...
            <div id="question-view" class="{{ request()->query('question_id') ? '' : 'hidden' }}">
                @if(request()->query('question_id') && isset($question))
                    <a href="{{ route('user.course.watch', ['course' => $course->id, 'module' => $current_module_id, 'tab' => 'qa']) }}" class="flex items-center gap-2 mb-4 sm:mb-6 text-orange-600 hover:text-orange-700 font-medium text-sm sm:text-base">
                    ...
                    <div class="bg-gray-50 rounded-xl p-4 sm:p-6 mt-6 relative">
                        <form method="POST" id="reply-form" action="{{ route('user.course.replies.store', $course->id) }}">
                            @csrf
                        ...
                        <div id="reply-form-loading" class="hidden loading-overlay absolute inset-0 bg-gray-50 bg-opacity-75 flex items-center justify-center">
                            <div class="flex items-center gap-3 text-orange-600">
                                <svg class="animate-spin h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 0 018-8v8H4z"></path>
                                </svg>
    ...
    <!-- Notes Tab Content -->
    @if(request()->query('tab', 'qa') == 'notes')
        <div id="notes-tab" class="tab-content">
            <div class="space-y-4 sm:space-y-6">
                <div id="no-notes-card" class="{{ $notes->isEmpty() ? '' : 'hidden' }} text-center py-8 sm:py-12">
                    <i class="mdi mdi-note-text text-5xl sm:text-6xl text-gray-300 mb-4"></i>
                    <h3 class="text-lg sm:text-xl font-semibold text-gray-600 mb-2">Personal Notes</h3>
                    <p class="text-gray-500 text-sm sm:text-base mb-4 sm:mb-6">Keep track of important concepts and insights</p>
                    <button onclick="toggleNoteForm()" id="create-note-btn" class="btn-primary text-white px-6 sm:px-8 py-2 sm:py-3 rounded-full font-medium text-sm sm:text-base transition-all duration-300">
                        Create First Note
                    </button>
                </div>

                <!-- Create Note Form -->
                <div id="create-note-form" class="hidden bg-gray-50 rounded-xl p-4 sm:p-6 mb-6 sm:mb-8 relative">
                    <form method="POST" id="note-form" action="{{ route('user.course.notes.store', $course->id) }}">
                        @csrf
                        <input type="hidden" name="module_id" value="{{ $current_module_id }}">

                        <div class="mb-4">
                            <input type="text" name="title" id="note-title" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="Note title" value="{{ old('title') }}">
                            <span class="text-red-500 text-xs hidden error-message" id="note-title-error">{{ $errors->first('title') }}</span>
                        </div>

                        <div class="mb-4">
                            <textarea name="note" id="note-content" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500" rows="4" placeholder="Write your note...">{{ old('content') }}</textarea>
                            <span class="text-red-500 text-xs hidden error-message" id="note-content-error">{{ $errors->first('content') }}</span>
                        </div>

                        <div class="flex justify-end gap-3">
                            <button type="button" onclick="toggleNoteForm()" class="bg-gray-200 text-gray-700 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-sm sm:text-base">
                                Cancel
                            </button>

                            <button type="submit" class="btn-primary text-white px-6 sm:px-8 py-2 sm:py-3 rounded-full font-medium text-sm sm:text-base transition-all duration-300 flex items-center gap-2" id="note-submit-btn">
                                <span class="submit-text">Save Note</span>
                                <svg class="loading-spinner hidden animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
                                </svg>
                    ...
                    <!-- Note form loading overlay -->
                    <div id="note-form-loading" class="hidden loading-overlay absolute inset-0 bg-gray-50 bg-opacity-75 flex items-center justify-center">
                        <div class="flex items-center gap-3 text-orange-600">
                            <svg class="animate-spin h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
                            </svg>
                            <span class="font-medium">Saving your note...</span>
                        </div>
                    </div>
                </div>

                @if(!$notes->isEmpty())
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0" id="notes-header">
                        <h2 class="text-lg sm:text-xl font-bold text-gray-800">
                            My Notes <span id="note-count" class="text-orange-600 ml-1">{{ $notes->count() }}</span>
                        </h2>

                        <button onclick="toggleNoteForm()" id="create-note-btn" class="btn-primary text-white px-4 sm:px-6 py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300">
                            Add Note
                        </button>
                    </div>
                @endif
                ...
                <!-- Notes List -->
                <div id="notes-container" class="space-y-4 sm:space-y-6 overflow-y-auto smooth-scroll max-h-[60vh] {{ $notes->isEmpty() ? 'hidden' : '' }}">
                    @include('user.courses.watch.navigation.notes', ['notes' => $notes])
                </div>
            </div>
        </div>
    @endif
...
<script>
    ...
    const currentModuleId = {{ $current_module_id }};
    const currentUserId = {{ auth()->id() }};
    const csrfToken = '{{ csrf_token() }}';
    const replyAction = '{{ route("user.course.replies.store", $course->id) }}';
    const questionsAction = '{{ route("user.course.questions.fetch", ["course" => $course->id]) }}';
    const backToQuestionsAction = '{{ route('user.course.watch', ['course' => $course->id, 'module' => $current_module_id, 'tab' => 'qa']) }}';
    let currentQuestionId = '{{ request()->query("question_id") ?? null }}';
    const originals = new Map();

    // Action URLs
    const questionUpdateAction = (id) => '{{ route("user.course.questions.update", ["course" => $course->id, "question" => "ID"]) }}'.replace('ID', id);
    const questionDeleteAction = (id) => '{{ route("user.course.questions.destroy", ["course" => $course->id, "question" => "ID"]) }}'.replace('ID', id);
    const replyUpdateAction = (id) => '{{ route("user.course.replies.update", ["course" => $course->id, "reply" => "ID"]) }}'.replace('ID', id);
    const replyDeleteAction = (id) => '{{ route("user.course.replies.destroy", ["course" => $course->id, "reply" => "ID"]) }}'.replace('ID', id);
    const noteUpdateAction = (id) => '{{ route("user.course.notes.update", ["course" => $course->id, "note" => "ID"]) }}'.replace('ID', id);
    const noteDeleteAction = (id) => '{{ route("user.course.notes.destroy", ["course" => $course->id, "note" => "ID"]) }}'.replace('ID', id);
    ...
    function toggleNoteForm() {
        ...
        const noNotesCard = document.getElementById('no-notes-card');
        ...
        const noteCount = noteCountElement ? parseInt(noteCountElement.textContent || 0) : 0;

        if (form.classList.contains('hidden')) {
            form.classList.remove('hidden');
            if (noteCount === 0) {
                noNotesCard.classList.add('hidden');
            }
            btn.textContent = 'Cancel';
            ...
            form.classList.add('hidden');
            if (noteCount === 0) {
                noNotesCard.classList.remove('hidden');
                if (notesHeader) notesHeader.classList.add('hidden');
                notesContainer.classList.add('hidden');
            }
            btn.textContent = noteCount === 0 ? 'Create First Note' : 'Add Note';
    ...
    function clearQuestionForm() {
        ...
        document.querySelectorAll('#ask-question-form .error-message').forEach(error => {
    ...
    function clearNoteForm() {
        ...
        document.querySelectorAll('#create-note-form .error-message').forEach(error => {
            error.classList.add('hidden');
        });
    }
    ...
    async function submitQuestionForm(e) {
        ...
        const content = formData.get('question').trim();
        ...
        let hasError = false;
        if (!title) {
            document.getElementById('title-error').textContent = 'Question title is required';
            document.getElementById('title-error').classList.remove('hidden');
            hasError = true;
        }
        if (!content) {
            document.getElementById('question-content-error').textContent = 'Question content is required';
        ...
        showLoading('question-form-loading', 'question-submit-btn');
        ...
        try {
            ...
            const data = await response.json();
            console.log('Question response:', data); // Debug log

            if (response.ok) {
                // Create a new question element
                const newQuestion = createQuestionElement(data.question);
                const container = document.getElementById('questions-container');
                container.insertAdjacentHTML('afterbegin', newQuestion);
                ...
                // Remove no-questions if present
            ...
            document.getElementById('question-content-error').textContent = error.message || 'Error posting question. Please try again.';
            document.getElementById('question-content-error').classList.remove('hidden');
        } finally {
            hideLoading('question-form-loading', 'question-submit-btn');
        }
    }
    ...
    async function submitNoteForm(e) {
        ...
        const formData = new FormData(form);
        const title = formData.get('title').trim();
        const content = formData.get('note').trim();
        ...
        let hasError = false;
        if (!title) {
            document.getElementById('note-title-error').textContent = 'Note title is required';
            document.getElementById('note-title-error').classList.remove('hidden');
            hasError = true;
        }
        if (!content) {
            document.getElementById('note-content-error').textContent = 'Note content is required';
        ...
        showLoading('note-form-loading', 'note-submit-btn');
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (currentCount === 0) {
                    ...
                    if (!notesHeader) {
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0" id="notes-header">
                            <h2 class="text-lg sm:text-xl font-bold text-gray-800">
                                My Notes <span id="note-count" class="text-orange-600 ml-1">1</span>
                            </h2>
                            <button onclick="toggleNoteForm()" id="create-note-btn" class="btn-primary text-white px-4 sm:px-6 py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300">
                                Add Note
                            </button>
            ...
            document.getElementById('note-content-error').textContent = error.message || 'Error saving note. Please try again.';
            document.getElementById('note-content-error').classList.remove('hidden');
        } finally {
            hideLoading('note-form-loading', 'note-submit-btn');
        }
    }

    // Create question element HTML
    function createQuestionElement(question) {
        const isOwner = question.user && question.user.id === currentUserId;
        const truncatedContent = question.content.length > 200 ? question.content.substring(0, 200) + '...' : question.content;
        return `
        <div id="question-${question.id}" class="bg-white rounded-xl p-4 sm:p-6 shadow-sm border hover:shadow-md transition-all duration-300">
            <div class="flex items-start gap-3 sm:gap-4">
                <img class="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover" src="${getProfileImage(question.user)}" alt="${question.user.name}">
                <div class="flex-1">
                    <h3 class="font-semibold text-gray-800 text-base sm:text-lg mb-2">${question.title}</h3>
                    <p class="text-gray-600 text-sm sm:text-base mb-3">${truncatedContent}</p>
                    ...
                    ${isOwner ? `
                    <div class="mt-3 flex gap-3">
                        <button onclick="editQuestion(${question.id})" class="text-blue-600 hover:text-blue-700 text-sm flex items-center gap-1"><i class="mdi mdi-pencil"></i> Edit</button>
                        <button onclick="deleteQuestion(${question.id})" class="text-red-600 hover:text-red-700 text-sm flex items-center gap-1"><i class="mdi mdi-delete"></i> Delete</button>
                    </div>
                    ` : ''}
                </div>
                <button onclick="viewQuestion(${question.id})" class="relative p-2 text-orange-600 hover:bg-orange-50 rounded-lg transition-colors min-w-[40px] sm:min-w-[48px]">
                    ...
                    <span class="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center">${question.replies_count}</span>
                </button>
            </div>
        </div>
    `;
    ...
    function createNoteElement(note) {
        ...
        <div id="note-${note.id}" class="bg-white rounded-xl p-4 sm:p-6 shadow-sm border hover:shadow-md transition-all duration-300">
            ...
            <div class="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm">
                <span class="text-gray-500">${note.created_at_diff}</span>
                <span class="text-gray-400">•</span>
                <span class="text-gray-500">${note.module.title}</span>
            </div>
            <div class="mt-3 flex gap-3">
                <button onclick="editNote(${note.id})" class="text-blue-600 hover:text-blue-700 text-sm flex items-center gap-1"><i class="mdi mdi-pencil"></i> Edit</button>
                <button onclick="deleteNote(${note.id})" class="text-red-600 hover:text-red-700 text-sm flex items-center gap-1"><i class="mdi mdi-delete"></i> Delete</button>
            </div>
        </div>
    `;
    }
    ...
    `;
        ...
        try {
            const url = `{{ route('user.course.watch', ['course' => $course->id, 'module' => $current_module_id]) }}?tab=qa&question_id=${id}`;
            ...
            if (response.ok && data.question) {
                ...
                const isOwner = user && user.id === currentUserId;
    ...
    function editQuestion(id) {
        const isSingle = document.getElementById('questions-view').classList.contains('hidden');
        const elemId = isSingle ? 'question-detail' : `question-${id}`;
        const elem = document.getElementById(elemId);
        if (!elem) return;
        ...
        <div class="relative p-4 sm:p-6">
            <form id="edit-question-form-${id}" action="${questionUpdateAction(id)}">
                <input type="hidden" name="_token" value="${csrfToken}">
                <input type="hidden" name="_method" value="PUT">
                <div class="mb-4">
                    <input type="text" name="title" value="${title}" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500">
                    <span class="text-red-500 text-xs hidden error-message" id="edit-title-error-${id}"></span>
                </div>
                <div class="mb-4">
                    <textarea name="question" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500" rows="4">${content}</textarea>
    ...
    `;
        ...
        document.getElementById(`edit-question-form-${id}`).addEventListener('submit', (e) => submitEditQuestion(e, id, elemId, isSingle));
    }

    // Submit edit question
    async function submitEditQuestion(e, id, elemId, isSingle) {
        e.preventDefault();
        const form = e.target;
        const formData = new FormData(form);
        const title = formData.get('title').trim();
        ...
        let hasError = false;
        document.getElementById(`edit-title-error-${id}`).classList.add('hidden');
        document.getElementById(`edit-content-error-${id}`).classList.add('hidden');

        if (!title) {
            document.getElementById(`edit-title-error-${id}`).textContent = 'Question title is required';
            document.getElementById(`edit-title-error-${id}`).classList.remove('hidden');
            hasError = true;
        }
        if (!content) {
            document.getElementById(`edit-content-error-${id}`).textContent = 'Question content is required';
        ...
        try {
            ...
            if (response.ok) {
                ...
                `;
                    ...
                    elem.outerHTML = createQuestionElement(data.question);
                }
                originals.delete(`question-${id}`);
            } else {
                document.getElementById(`edit-content-error-${id}`).textContent = data.message || 'Failed to update question';
                document.getElementById(`edit-content-error-${id}`).classList.remove('hidden');
            }
        } catch (error) {
            console.error('Error updating question:', error);
            document.getElementById(`edit-content-error-${id}`).textContent = 'Error updating question. Please try again.';
            document.getElementById(`edit-content-error-${id}`).classList.remove('hidden');
        } finally {
    ...
    function handleDeleteClick(e, actionUrl, type, id, countElementId, containerId, noItemId, singleViewRedirect = null, callback = null) {
        e.preventDefault();
        iziToast.question({
            timeout: false,
            close: false,
                        ...
                        if (response.ok) {
                            if (singleViewRedirect) {
                                ...
                                if (type === 'note' && count === 0) {
                                    document.getElementById('no-notes-card').classList.remove('hidden');
                                    document.getElementById('create-note-btn').textContent = 'Create First Note';
                                }
    ...
    function deleteQuestion(id) {
        handleDeleteClick(
            event,
            questionDeleteAction(id),
            'question',
            id,
            'question-count',
            'questions-container',
            'no-questions',
            document.getElementById('questions-view').classList.contains('hidden') ? backToQuestionsAction : null
        );
    }
    ...
    function editNote(id) {
        ...
        elem.innerHTML = `
        <div class="relative p-4 sm:p-6">
            <form id="edit-note-form-${id}" action="${noteUpdateAction(id)}">
                <input type="hidden" name="_token" value="${csrfToken}">
                <input type="hidden" name="_method" value="PUT">
                <div class="mb-4">
                    <input type="text" name="title" value="${title}" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500">
                ...
                <div class="mb-4">
                    <textarea name="note" class="w-full text-gray-700 border border-gray-200 rounded-lg p-3 sm:p-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-orange-500" rows="4">${content}</textarea>
                    <span class="text-red-500 text-xs hidden error-message" id="edit-content-error-${id}"></span>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="cancelEdit('${key}', '${elemId}')" class="bg-gray-200 text-gray-700 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-sm sm:text-base">
    ...
    `;
        ...
        document.getElementById(`edit-note-form-${id}`).addEventListener('submit', (e) => submitEditNote(e, id, elemId));
    }

    // Submit edit note
    async function submitEditNote(e, id, elemId) {
        e.preventDefault();
        const form = e.target;
        const formData = new FormData(form);
        const title = formData.get('title').trim();
        ...
        if (!title) {
            document.getElementById(`edit-title-error-${id}`).textContent = 'Note title is required';
            document.getElementById(`edit-title-error-${id}`).classList.remove('hidden');
            hasError = true;
        }
        if (!content) {
            document.getElementById(`edit-content-error-${id}`).textContent = 'Note content is required';
            document.getElementById(`edit-content-error-${id}`).classList.remove('hidden');
            hasError = true;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                document.getElementById(`edit-content-error-${id}`).textContent = data.message || 'Failed to update note';
                document.getElementById(`edit-content-error-${id}`).classList.remove('hidden');
            }
        } catch (error) {
            console.error('Error updating note:', error);
            document.getElementById(`edit-content-error-${id}`).textContent = 'Error updating note. Please try again.';
            document.getElementById(`edit-content-error-${id}`).classList.remove('hidden');
        } finally {
            hideLoading(`edit-loading-${id}`, `edit-submit-${id}`);
        }
